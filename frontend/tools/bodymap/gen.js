...
// ---------- region bounds (for the focus-frame camera) ----------
// From the raw point extremes, padded for Catmull-Rom overshoot. All shapes
// are symmetric (pair/half), so x spans [minX, 200 - minX].
const BEZIER_PAD = 3;
function regionBounds(region) {
  ...
  return {
    x0: r1(Math.max(0, minX - BEZIER_PAD)),
    y0: r1(Math.max(0, minY - BEZIER_PAD)),
    x1: r1(Math.min(200, 200 - minX + BEZIER_PAD)),
    y1: r1(Math.min(440, maxY + BEZIER_PAD)),
  };
}
...
// Machine-readable copy of the asset for QA tooling (gitignored).
function buildPathsJson() {
  const out = { viewBox: '0 0 200 440', outline: { front: OUTLINE_PATH, back: OUTLINE_PATH }, front: {}, back: {} };
  for (const [view, regions] of Object.entries(VIEWS)) {
    for (const [key, region] of Object.entries(regions)) {
      out[view][key] = { group: region.group, bounds: regionBounds(region), path: regionPath(region) };
    }
  }
  return out;
}
...
fs.writeFileSync(path.join(__dirname, 'paths.json'), JSON.stringify(buildPathsJson(), null, 2));
fs.writeFileSync(
  path.join(__dirname, '..', '..', 'src', 'components', 'bodymap', 'bodyMapPaths.ts'),
  emitTs()
);
console.log('wrote preview.html, detail.html, paths.json and src/components/bodymap/bodyMapPaths.ts');
