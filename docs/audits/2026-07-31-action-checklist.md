...
- [x] **1.4 Success haptics on set-complete and finish** `[OTA]` **S** — **DONE** (`buzzSetComplete` / `buzzAllSetsComplete` in `planCalendarPrototype.ts`). The table at the top of this file already said BUILT; the box was never ticked.
...
- [x] **2.5 Resolve the dead "+ Add RPE" menu item** `[OTA]` **S** — **RESOLVED BY REMOVAL 2026-08-27.** It lived in `WorkoutSession.tsx`, which was deleted (3,501 lines, zero importers). Nothing in the live calendar session offers RPE, so there is no dead affordance left to resolve.
...
- [x] **4.2 False empty states from catch blocks** `[OTA]` **S** — **DONE 2026-08-27 (`8a89790`).** `ExerciseDetailScreen` swallowed the error and rendered "Exercise not found", a claim about the CATALOG made when the server was simply unreachable; it now distinguishes the two and offers Try again. The `WorkoutScreen.tsx` half died with that file. Swept the remaining screens for the same shape: CrewScreen's catches all set an offline flag, and the exercise picker's are deliberate (the library below renders its own state).
...
- [x] **4.4 Cross-account draft leak** `[OTA]` **S** — **RESOLVED BY REMOVAL 2026-08-27.** `workoutDraftStorage.ts` was deleted with the dead session; nothing reads the device-global key any more. ⚠ The AsyncStorage key `jim_workout_draft_v1` may still hold bytes on devices that ran the old session — inert, but it is another user's data at rest, so if a draft system ever returns it must be per-user AND must clear that legacy key.
- [ ] **4.5 Preview "Swap Workout" creates a dead slot that survives Apply** `[OTA]` **S–M** — ⚠ **CONFIRMED end-to-end 2026-08-27, and it needs a PRODUCT DECISION before anyone codes it.**

  Mechanism: `handleReplaceWithType` writes a new workout card into `planData` (what the preview DISPLAYS) but sets `session: null` in `planDraft` (what Apply reads). `handleApply` then iterates `planData` for the slot while resolving its exercises via `slotExercisesFromDraft(planDraft, …)` — which now returns nothing. The slot is created with NO exercises.

  ⚠ Worse than "Start dead-ends": `scheduledSlotOn` requires `hasExercises`, so an exercise-less slot is **invisible to the calendar and to the crew streak** — it is not a workout the system can see at all.

  The decision, which is why this is not just a bug fix: **what should a type-swap produce?** (a) regenerate real exercises for the new type — the machinery exists (`regeneratePipelineWeek`, `regeneratePipelineCardioSessions`) but it is a network call inside a modal, with a failure path; (b) allow genuinely exercise-less cardio/recovery slots and teach `hasExercises` about them; (c) drop the affordance. Every option changes product behaviour, so the desync was left in place rather than guessed at.
- [x] **4.6 Preview move-to-occupied-day desyncs UI from draft** `[OTA]` **S–M** — **DONE 2026-08-27.** Confirmed exactly: `handleMoveToDay` ran TWO different operations — the display APPENDED to the destination, the draft SWAPPED the two days' sessions. They agree only when the destination is empty, which is why it survived. Onto an occupied day, `handleApply` (slots from `planData`, exercises from `planDraft`) wrote the destination twice and lost the origin's session. Both stores now share one definition, `lib/planPreviewMove.ts`, extracted so the operation is unit-tested (8 cases); the swap is correct, since a day holds one session and the calendar already says "make room".
...
- [x] **4.8 e1RM trend chart illegible by construction** `[OTA]` **S** — **DONE 2026-08-27 (`38c857d`).** The domain now fits the data. Since zooming a y-axis exaggerates, the cost is disclosed rather than hidden: the label carries the range ("Estimated 1RM · 158–169 lb"), and a genuinely flat trend draws flat instead of manufacturing a slope.
...
- [x] **4.10 Calendar month fetch failure: no retry, falsely-empty grid** `[OTA]` **S** — **DONE** in the loading-states sweep; `CalendarScreen` sets `logsError` and renders a connection message instead of an empty grid.
...
- [x] **5.2 Rep-PRs and e1RM-PRs** — **DONE 2026-08-27 (`a8aa0b0`).** ⚠ The premise was overstated: reps already counted, but only as a tie-break at IDENTICAL weight, so the invisible case was a PR won on a LIGHTER bar. Fixed full-stack and ADDITIVELY: the server now serves a second record (strongest set by e1RM) beside the existing heaviest-bar one, from the same query, and `results` keeps its meaning so shipped builds are unaffected. New `estimated` basis, checked only after the heavier-bar test so a real weight PR is never downgraded.
