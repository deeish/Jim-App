...
| 9 | "Recently trained" layer (trained heat) | `DONE` | backend `workout-logs/muscle-heat.ts` (+spec), `GET /workout-logs/muscle-heat?days=`, `MuscleHeatQueryDto`; frontend `lib/muscleHeat.ts` (+test), `workoutService.getMuscleHeat`, `MuscleExplorer.tsx` | Backend: pure model over the user's logs in the window (default 7 days, max 28): every completed set = 1 for the exercise's sub-muscles (whole primary group when a row has none) and 0.5 for each sub-muscle of its secondary groups, decayed with a 3.5-day half-life, saturating into 0..1 (`1 − e^(−score/10)`); returns per sub-muscle score, intensity, direct sets, assist sets, lastTrainedAt, hottest first. Group→sub map mirrors the client's hierarchy — three copies, keep in step. `WorkoutLogsService` now injects `ExercisesService` (specs stubbed). Frontend: a second switch in the explorer's control row (Explore · This week, beside Front · Back; Reset moved between them so it covers neither); the layer fetches once per mount, every region wears its own group hue at the trained alpha (floor 0.18 so a touched muscle is never invisible), the tapped one goes solid, the sheet gains "Trained today · 16 sets" / "Trained 4 days ago · assisted in 8" / "Not trained in the last 7 days"; captions for loading, error, and an empty week. ⚠ First cut never resolved: the fetch effect depended on its own status, so flipping to "loading" re-ran it and the cleanup cancelled the request — now one request per mount guarded by a ref, dropped only on unmount. Rig-verified dark + light with a mock endpoint, incl. the empty case. Dylan then: (1) "Explore" is a label for the off state and means nothing → replaced the two-segment control with ONE toggle pill (outlined off, filled with a check on); (2) "This week" was the wrong name — the model is a rolling, decaying window, not a calendar week (a Sunday session still lights Monday at ~80 %), and it read like a stats period → named **Recently trained** from a shortlist (Recent work / Trained / Last 7 days / Load / Recovery — the last belongs to the future sore layer). Sheet empty line is "Not trained recently". |
| 11 | The retag: anatomical involvement per exercise | `DONE` | backend `data/muscle-regions.ts`, `data/exercise-muscle-involvement.ts` (+spec), `transformExercise` → `muscles`; frontend `exerciseToHighlights` (region path), `bodyMapFigure` (emphasis alpha), `bodyMapRegions.sync.test.ts` | Dylan (after "will a full retag be optimal?"): retag secondaries fully, primaries at head level only where the body agrees. Built as RULES over the catalog's existing tags, not 1,342 hand edits: (1) ~400 movement-pattern ids collapse into 35 families (`familiesOf`, pattern regex + name backstop; "upright row" is never a back row); (2) primaries expand to every head of their sub-muscle at 1, with emphasis only where the evidence is solid — leg curl → semitendinosus 1 / biceps femoris 0.6, hinge → the reverse (0.7), squat → vasti 1 / rectus 0.6, leg extension → rectus 1 / vasti 0.8, seated calves → soleus 1 / gastroc 0.4, standing → gastroc 1 / soleus 0.5, overhead triceps → long head 1 / others 0.7, pushdowns → lateral+medial 1 / long 0.6, hammer/reverse curl → brachialis 1, incline curl → long head, preacher → short head, lats → teres 0.6–0.8, outer thigh → glute med 1 / TFL 0.7, inner → adductor longus 1 / magnus 0.8 / gracilis 0.6; (3) secondary GROUPS resolve to muscles by family — pushing arms = triceps, pulling arms = biceps + forearms, pushing shoulders = front delts (side for overhead), pulling = rear delts, squat/hinge/carry back = lower back, core = abs + obliques (rotation = obliques), weights 0.2–0.5, never a region that is already a primary. Output per REGION (the figure's anatomical names) as `muscles: [{region, sub, role, weight}]` on every exercise payload. Measured on the real catalog: 1,289 rows carry sub-muscle tags (96 %), 1,161 carry secondary groups; cardio yields []. Spec: 10 cases incl. whole-catalog invariants (every non-cardio row has a primary, all names on the figure, no primary/secondary overlap). The client's figure now takes the region list when present (primaries keep emphasis 0.6–1 drawn in the hue at alpha 0.62–1, secondaries capped under the 0.5 threshold as the assist wash) and falls back to the old group mapping otherwise; a sync test reads the backend's region table and asserts it equals the asset's own tags. ⚠ `exercise-mappings.ts` ↔ `exercise-muscle-involvement.ts` import each other on purpose (call-time use only). |
| 12 | Heat per region | `DONE` | `muscle-heat.ts` (+spec), `lib/muscleHeat.ts`, `MuscleExplorer.tsx` | The heat model now scores each REGION by its involvement weight (a leg curl lands harder on the inner hamstring), `sets` = direct work (weight ≥ 0.6), `assistSets` otherwise; entries carry `region` + `muscle` (sub); the client keys by region first and falls back to the sub for older payloads. Rig-verified with a region-level mock: semitendinosus hotter than biceps femoris, soleus hotter than gastroc after a seated calf day. Tap-through now opens on the exercise's FIRST catalog sub-muscle (ties among equal-weight regions were landing on Glute Max for a squat). |
| 13 | Retag review page | `NEEDS-DYLAN` | artifact https://claude.ai/artifact/CgHAzmNUxtVuTPMRtWuikE (scratchpad `dump-involvement.ts` via ts-node + `make-retag-review.js`) | All 1,289 non-cardio rows, S→A→B→C→D→ungraded, old tags beside resolved regions with weights, search + group filter. Dylan reviews the S/A rows (130) first; corrections become per-exercise overrides on top of the rules. |
| 10 | Recovery layer proposal | `DONE` (built, row 14) | artifact only (scratchpad make-recovery.js + palette.js) | Chat round settled the direction: the trained layer becomes **Recovery** on ONE five-step sequential scale (OKLCH, amber→red, #F1AF57 #E88519 #D16022 #C12B11 #9B0E1E, ~20 ΔE per step, colour-blind safe, grey = fresh) instead of the group hues; driven by a recovery estimate (dose = sets × hardness × novelty × role; fatigue rises then falls, τ 24/32/40 h by muscle size; summed, saturated, cut at 8/28/48/68/85 %; calibrated by the finish-screen soreness answer); self-report is the correction (Still sore = top step fading 3 days, Feeling fine = clear), asked on the pre-workout card and corrected in the sheet — different verbs, not the same button twice; guidance is a range ("3–4 sets · go by feel") + one-tap Ease today, never a silent cut; the Muscles pill stays, relabelled. Dylan reviewed the artifact on 2026-09-24 ("looks good") and said build; see row 14. |
| 14 | Recovery layer BUILT | `DONE` | backend e1325ee (`workout-logs/muscle-recovery.ts` +spec, `dto/recovery-note.dto.ts`, service + controller, `MuscleRecoveryNote` model + migration `20260924100000_muscle_recovery_notes`); frontend cc0d0c7 (`lib/muscleRecovery.ts` +test, `workoutService`, `MuscleExplorer.tsx`, `components/RecoveryCard.tsx`, `HomeScreen`, `SearchScreen`, `types/navigation.ts`) | As proposed in row 10, in the agreed order. Backend: `computeMuscleRecovery` reads ten days of logs (42 for novelty) with the check-in soreness and per-set RPE; dose per region = sets × hardness (RPE/8, clamped 0.6–1.2) × novelty (1.4 when the exercise is new in 42 days) × involvement weight from the retag; each dose follows f(t) = (t/τ)·e^(1−t/τ) with τ 24/32/40 h by region size; the sum saturates 1−e^(−sum/8) and cuts at 8/28/48/68/85 % into Fresh · Lightly worked · Recovering · Working hard · Fatigued · Very fatigued; the soreness answer scales ±15 %. Notes: 'sore' pins the top step and fades over 3 days, 'fine' drops the earlier doses; one row per user+region (upsert), so a correction is idempotent. `GET /workout-logs/recovery?exerciseIds=` (touchedToday for the card), `POST recovery/notes`, `DELETE recovery/notes/:region`. 6 new specs, 120 workout-logs tests green, tsc clean. ⚠ `migrate:deploy` on Render BEFORE the next client build. Client: the Muscles pill is now **Recovery** (role switch), fills come from the five-step scale (`recoveryFillsByRegion`, grey when fresh or no data), captions for loading / failed / nothing logged, one fetch per mount; the sheet adds the estimate line ("Working hard · trained yesterday, 8 sets · fresh in about 3 days", bold from step 3) and the correction row (Feeling fine / Still sore → "You said still sore today" tag + Undo), state replaced from the server reply. Home shows `RecoveryCard` only before a planned, unstarted session whose exercises touch a region at step ≥3: "{names} is still recovering", the range line, chips with the step swatch, and "Not how it feels? See on the body" → Exercises tab, Muscles segment, Recovery on, framed on that region (`openRecovery` param). 5 lib tests, 72 frontend tests, tsc clean. Rig drive (`drive-recovery.js`, mock API with region payloads) ALL PASS in dark and light: quads painted / sartorius quiet, inner hamstring hotter than outer, sheet line, Still sore → note + top colour, Undo, Feeling fine → quiet, toggle off. NOT verified on web: Skia fills, native gestures, the Home card against a real plan (the rig has none). Deliberately NOT built: one-tap **Ease today** (needs the session prescription threaded through the card; the range line covers the advice for now), a Progress-tab card, per-exercise overrides (wait for Dylan's retag corrections, row 13). |
| 15 | Build 37 phone-pass findings (Dylan, 2026-09-24) | `DONE` | 11b85c8 `PlateSheet.tsx`; cc799dd `lib/setEntryRules.ts` (+test), `lib/plateMath.ts` (+test), `planCalendarPrototypeStore.ts` (+persistence test), `PlanCalendarWorkoutScreen.tsx`, `constants/changelog.ts` | Four findings, four fixes. (1) The plates sheet drew no bar: the sleeve the plates sit on had no fill, so a bare bar was two tips floating either side of a 40 pt shaft, invisible on a dark surface — it is a bar now (textTertiary), under the plates. (2) A dead hang demanded a weight: 'Bodyweight' was only assigned when the catalog equipment was empty or literally said bodyweight, so a pull-up bar made a LOADED row and the check refused a blank; `isBodyweightEquipment` now says a row is bodyweight when nothing on it can be loaded (bar to hang from, dip station, rings, box, mat), and both store paths use it. Plus a **BW chip** in the weight box on every loaded row (Dylan asked for 'a button on the keyboard'; a chip in the box is visible before the keyboard and works on Android): one tap logs the set at bodyweight, typing a number takes it off. (3) The − / + steppers are gone, `stepWeight` and its tests with them. (4) 'Still unable to edit a completed exercise': #57 was fixed on the FINISH screen's ledger, but the issue named the workout screen's SET BREAKDOWN — the grid cards are tappable now with the same inline editor (reps or time + weight, blank = bodyweight); before Complete Workout the edit is local only (`editSetLog`, so a session finished earlier the same day is never touched), after it the stored log is corrected through the existing path. The 1.5.0 What's New card edited in place (still unshipped externally). tsc clean, 62 suites / 817 tests. NOT rig-verified: the workout screen needs a plan the rig does not have; Dylan's build-38 pass covers it. |
| 6 | Tap-through from the exercise page | `DONE` | `ExerciseDetailScreen.tsx`, `SearchScreen.tsx`, `MuscleExplorer.tsx`, `bodyMapRegions.ts`, `types/navigation.ts` | The Muscles card's figure is a button (plus an "Explore on the body ›" link in its legend row): it navigates to the Exercises tab's list with `openMuscle` = the strongest highlight; SearchScreen consumes the param into state, opens the Muscles segment and clears it, so a later plain visit starts at the whole body (the explorer is keyed on the request). `primaryRegionFor(name)` picks the largest region carrying the name on whichever view holds it (Quads → Rectus Femoris, front); the explorer selects it once the stage has a size, exactly as a tap would. Web-verified with a mock API (list → page → link → framed on the quads; plain revisit resets). |
| 7 | Figure follows the theme | `DONE` | `bodyMapFigure.ts`, `MuscleBodyMap(.web).tsx`, `MuscleBodyTile.tsx` | `buildBodyMapFigure` took only the light palette, so dark mode drew the pale body on dark cards; renderers now pass `useTheme().colors` + `mode`. Knock-on: the "also works" wash (hue at 25 %) was tuned for the pale body and nearly vanished on the dark one → 45 % in dark (`assistColorFor(hex, mode)`, legend dot + chips follow). ⚠ First "verification" used a mock with a sub-muscle as a secondary, which the highlight mapper drops (secondaries are parent groups) — the back figure was untinted for a data reason, not a colour one; re-verified with catalog-shaped data, 1.7k tinted px on the back figure in dark. |
| 8 | Mini tiles: silhouette + lit muscle only | `DONE` | `MuscleBodyTile.tsx` (`hideQuiet` below 60 pt) | At 44 pt the quiet anatomy was noise, especially legs/calves; tiles now draw the body and the lit regions only. |
