...
| 8 | Build 1.3.0 (33) → TestFlight (internal) | `DONE` (queued) | EAS build `361afc3d`, submission `4070f395` | Cut 2026-09-15 22:05 with the non-interactive recipe (skew shim + ASC key), `--auto-submit`. Internal testers get it when Apple finishes processing; the Friends/Family group is a separate `tf:distribute` step, not done. First binary with HealthKit: watch the signing step for the "we aren't authenticated" profile skip from memory. |
