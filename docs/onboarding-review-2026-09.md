...
|---|------------------------|----------|-----|
| 1 | **"Build a custom plan with AI" is not AI right now.** `GROQ_MODEL` in `workout-generator.service.ts` still names `llama-3.3-70b-versatile`, retired by Groq on 2026-08-16. Every generation falls to the rule-based builder; the fallback is reported to logs and Sentry only, never to the user. Home says "Generate a personalized week with AI". | High | Swap the model (Dylan is choosing) before the AI exit is promoted, or make the copy honest until then ("Build a custom plan"). Show a one-line note in the preview when a plan came from the rule-based builder — the flag already exists server-side. |
